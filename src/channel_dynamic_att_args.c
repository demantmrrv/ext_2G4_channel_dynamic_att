...
#define DYNAMIC_DEFAULT_ATT (60)
#define DYNAMIC_ATT_MIN     (-100.0)
#define DYNAMIC_ATT_MAX     (100.0)
#define DYNAMIC_DEFAULT_PIPE_NAME "dynamic_att.dtc"
...
void component_print_post_help()
{
    fprintf(stdout, "This is a non realistic channel model.\n"
            "It models NxN independent paths each with on demand attenuation\n"
            "By default the overall attenuation is %d dBm. However, it can be change\n"
            "dynamically and independently for each connection.\n",
            DYNAMIC_DEFAULT_ATT
            );
}
...
void channel_dynamic_att_argparse(int argc, char *argv[], ch_dynamic_att_args_t *args)
{
    bs_args_struct_t args_struct[] = {
        ...
        {false, false, false,  "pipe",  "pipe_name",     's',        (void *)&args->pipe_name,                 NULL,
         "Name of pipe for communication."                                       },
        ARG_TABLE_ENDMARKER
    };

    args->default_attenuation = DYNAMIC_DEFAULT_ATT;
    args->pipe_name           = DYNAMIC_DEFAULT_PIPE_NAME;

    bs_args_override_exe_name(library_name);
    bs_args_set_trace_prefix("channel: (on_demand_att) ");
    bs_args_parse_all_cmd_line(argc, argv, args_struct);


    if ((args->default_attenuation < DYNAMIC_ATT_MIN) || (args->default_attenuation > DYNAMIC_ATT_MAX)) {
        bs_trace_error("channel: cmdarg: attenuation can only be between %lf dBm and %lf dBm (is %lf)\n",
